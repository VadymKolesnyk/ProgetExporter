...
export async function loadReleaseEntity(token: string, cardNumber: string) {
  const response = await fetch(
    `https://cors-anywhere-ten-zeta.vercel.app/smartwebs.tpondemand.com/api/v2/Release/${cardNumber}?` +
      new URLSearchParams({
...
export async function loadEntity(token: string, type: 'Bug' | 'UserStory' | 'Feature', cardNumber: string) {
  const response = await fetch(
    `https://cors-anywhere-ten-zeta.vercel.app/smartwebs.tpondemand.com/api/v2/${type}/${cardNumber}?` +
      new URLSearchParams({
